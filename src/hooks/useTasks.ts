...
interface UseTasksState {
  ...
  undoDelete: () => void;
  dismissUndo: () => void;   //  Added for Bug 2
}
...
export function useTasks(): UseTasksState {
  ...
  //  BUG 1 FIX: The second useEffect has been completely removed.
  ...
  //  BUG 2 FIX: Added this function to clear the "Undo" memory
  const dismissUndo = useCallback(() => {
    setLastDeleted(null);
  }, []);

  return { tasks, loading, error, derivedSorted, metrics, lastDeleted, addTask, updateTask, deleteTask, undoDelete, dismissUndo };  //  Added to return
}
